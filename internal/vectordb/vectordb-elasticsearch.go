...
func CreateKibana() Kibana {
	config := Kibana{
		...
		Environment: KibanaEnvironment{
			XPACK_ENCRYPTEDSAVEDOBJECTS_ENCRYPTIONKEY: "${XPACK_ENCRYPTEDSAVEDOBJECTS_ENCRYPTIONKEY:-d1a66dfd-c4d3-4a0a-8290-2abcb83ab3aa}",
			NO_PROXY:                          "${NO_PROXY:-localhost,127.0.0.1,elasticsearch,kibana}",
			XPACK_SECURITY_ENABLED:            "${XPACK_SECURITY_ENABLED:-true}",
			XPACK_SECURITY_ENROLLMENT_ENABLED: "${XPACK_SECURITY_ENROLLMENT_ENABLED:-false}",
			XPACK_SECURITY_HTTP_SSL_ENABLED:   "${XPACK_SECURITY_HTTP_SSL_ENABLED:-false}",
			XPACK_FLEET_ISAIRGAPPED:           "${XPACK_FLEET_ISAIRGAPPED:-true}",
			I18N_LOCALE:                       "${I18N_LOCALE:-zh-CN}",
